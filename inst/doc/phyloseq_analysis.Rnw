...
\begin{figure}[htbp]
\centering
\includegraphics[width=1.0\textwidth]{phyloseq_analysis-richness_estimates}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\emph{phyloseq} also contains a method for easily annotating a phylogenetic tree with information regarding the sample in which a particular taxa was observed, and optionally the number of individuals that were observed. 
...
plot_tree_phyloseq(GP.chl, color_factor="SampleType",
			type_abundance_value=TRUE, treeTitle="")
@
...
\begin{figure}[htbp]
\centering
\includegraphics[trim = 10mm 10mm 10mm 15mm, clip, width=0.6\textwidth]{phyloseq_analysis-EntAbundPlot}
...
<<entbarplot0, include=FALSE, fig=FALSE, height=6, width=8>>=
